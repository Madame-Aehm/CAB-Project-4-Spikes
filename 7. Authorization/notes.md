...
Following along with the documentation, we'll need to configure our Passport strategy in our app. Create a new `.js` file in `config` for Passport. We'll wrap this in a function to export the same way we did for Cloudinary configuration. This is just to keep our `index.js` as organised as possible. There is an example configuration in the [strategy documentation](https://www.passportjs.org/packages/passport-jwt/) that we can use as a guide: let's look at the function step by step. 

The first thing they're doing is setting some options. We're going to need to set the 'jwtFromRequest' property, which establishes how we'll be passing the token back to the server. In our case, as a **Bearer Token** in the Header, so we'll use `ExtractJwt.fromAuthHeaderAsBearerToken()` (the same as the sample code). We'll also need to set our `JWT_SECRET` from our `.env` file as the value for 'secretOrKey' property.
...
The next section does quite a few things:
  ...
  - This constructor receives as arguments the options we already defined above, along with a callback function
...
Create the export function, call it something like 'configurePassport', then paste the example code inside. Unfortunately for us, the newest version of Mongoose no longer allows callbacks in their functions, but it seems Passport haven't updated their documentation. This means we're going to have to change a few things:
  ...
  - We could separate the `new JwtStrategy()` into a separate variable to make everything a little bit more readable:
...
Import this function into the server `index.js` and call it after the Passport initialization. We've now initialized and configured Passport in our app!
